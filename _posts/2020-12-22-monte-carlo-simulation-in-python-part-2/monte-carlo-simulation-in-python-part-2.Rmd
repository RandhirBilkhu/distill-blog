...
Firstly I start by creating a function called `__init__` this method will be called any time an object is created from the class and will allow the class to initialize the attributes of the class
Note that inside `__init__` I have two arguments, self and pandas_obj. self is a reference to the current instance of the class (note you dont have to use the word 'self' but it is the convention. )
The second is pandas_obj as I am going to be passing a dataframe into this class.
